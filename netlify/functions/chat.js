...
exports.handler = async (event) => {
  ...
  try {
    ...
    let payload = {};
    try { payload = JSON.parse(event.body || "{}"); } catch { return text("JSON inválido en el body.", 400); }
    ...
    if (!Array.isArray(messages) || messages.length === 0) return text("Falta 'messages' (array).", 400);
    const rawBody = JSON.stringify(payload);
    if (rawBody.length > 2_000_000) return text("Payload demasiado grande.", 413);

    const groqBody = { model, messages, temperature };
    Object.assign(groqBody, pickDefined({ max_tokens, top_p, response_format, seed, stop, user },
      ["max_tokens", "top_p", "response_format", "seed", "stop", "user"]));
    ...
    return { statusCode: resp.ok ? 200 : resp.status, headers: { ...cors(), "Content-Type": "application/json" }, body: textResp || "" };
  } catch (err) {
